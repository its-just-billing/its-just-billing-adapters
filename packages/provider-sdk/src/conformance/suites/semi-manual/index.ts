...
import { registerPurchasesSemiManualSuite } from './purchases.js';
 ...
 * checkout, complete a purchase) and then assert on the resulting state.
 *
...
export function registerSemiManualSuite(label: string, factory: HarnessFactory): void {
  describe.skipIf(!isInteractiveMode())(`[semi-manual] ${label}`, () => {
    registerPurchasesSemiManualSuite(label, factory);
  });
}
