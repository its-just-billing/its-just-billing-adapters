...
- **fixture**: **subscriptions only.** A subscription is the one resource the public SDK can't bootstrap (it needs a checkout/payment), so it is the only pre-provisioned fixture — declared on `harness.fixtures.subscriptionId`. Every other resource is SDK-creatable; its lifecycle belongs in the **automated**/**self-setup** tiers (create at test time, archive after), NOT here. Do not add product/price/customer/discount/webhook fixture scenarios — they were audited as fully redundant with automated and removed. Fixture scenarios health-check the subscription's clean starting state (status `active|trialing`, `cancelAtPeriodEnd:false`, no pending change, one item), perform a reversible op, and revert; any scaffolding they need (e.g. a swap-target price) is created at test time and archived in `afterAll`. Test code uses `withFixture(key, { healthCheck, test, revert })` from `@its-just-billing/provider-sdk/conformance`. The subscription is reused across runs — there is no teardown; the harness factory is memoized. Providers that can create subscriptions programmatically (mock; Stripe via `setup.createSubscription`) cover this through self-setup and supply no fixture (tests skip). Providers that can't (Paddle/Polar) hand-commit `{ "subscriptionId": "..." }` in their package — see `packages/provider-stripe/README.md`.
