...
_TODO: handwrite a 1–2 paragraph description of what `discounts.list` does, when callers reach for it, and any gotchas._

## Request

See [`docs/openapi/discounts.json`](../../openapi/discounts.json) → operation `discounts.list` → `requestBody`.

## Response

See [`docs/openapi/discounts.json`](../../openapi/discounts.json) → operation `discounts.list` → response `200`.
