...
_TODO: handwrite a 1–2 paragraph description of what `customers.update` does, when callers reach for it, and any gotchas._

## Request

See [`docs/openapi/customers.json`](../../openapi/customers.json) → operation `customers.update` → `requestBody`.

## Response

See [`docs/openapi/customers.json`](../../openapi/customers.json) → operation `customers.update` → response `200`.
